    ...
    public abstract class StateMachine  {
        ...
        protected class SubStateMachine {
            ...
            public bool Enter(Stack<State> states) {
                ...
                if (next.parentMachine != this) {
                    Console.WriteLine("Error in transition hierarchy.");
                    return false;
                } else {
                    currentState = next;
                    ...
                    if (states.Count > 0 && currentState.subMachine == null) {
                        Console.WriteLine("State in transition hierarchy doesn't have children.");
                        return false;
            ...
            public bool TransitionTo(Stack<State> states) {
                ...
                if (next.parentMachine != this) {
                    Console.WriteLine("Error in transition hierarchy on state " + next);
                    return false;
                }

                if (next == currentState) {
                    if (states.Count == 0) {
                        Console.WriteLine("Trying to transition to already active state.");
                        return false;
                    } else if (currentState.subMachine == null) {
                        Console.WriteLine("Trying to transition to non-existant hierarchy.");
                        return false;
                    } else {
                        states.Pop();
        ...
        protected class Transition {
            ...
            public bool IsPassable() {
                ...
                if (mode == TransitionMode.ConditionOnly) {
                    return cond();
                } else if (mode == TransitionMode.TimeOnly) {
                    return t;
                } else if (mode == TransitionMode.TimeAndCondition) {
                    return t && cond();
                } else {
                    return t || cond();
                }
